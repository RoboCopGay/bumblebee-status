...
class Module(bumblebee.engine.Module):
    ...
    def capacity(self, widget):
        ...
        # notificando
        if capacity==100 and open(os.popen('echo $HOME/.bin/battery/log').read().strip()).read().strip()!='20':
            os.system("notify-send -u low Battery 'Baterry complete'")
            open(os.popen('echo $HOME/.bin/battery/log').read().strip(), 'w').write('20')

        elif capacity==20 and open(os.popen('echo $HOME/.bin/battery/log').read().strip()).read().strip()=='20':
            os.system("notify-send -u low Battery 'Baterry low'")
            open(os.popen('echo $HOME/.bin/battery/log').read().strip(), 'w').write('10')

        elif capacity==10 and open(os.popen('echo $HOME/.bin/battery/log').read().strip()).read().strip()=='10':
            os.system("notify-send -u normal Battery 'Baterry low'")
            open(os.popen('echo $HOME/.bin/battery/log').read().strip(), 'w').write('5')

        elif capacity==5 and open(os.popen('echo $HOME/.bin/battery/log').read().strip()).read().strip()=='5':
            os.system("notify-send -u critical Battery 'Baterry critical'")
    ...
    def getCharge(self, widget):
        ...
        try:
            with open("{}/status".format(widget.name)) as f:
                ...
                pass
        return charge
# vim: tabstop=8 expandtab shiftwidth=4 os.ttabstop=4
